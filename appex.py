...
if output['last_clicked']:
    ...
    h2 = st.number_input("Enter the height (in meters) for wind speed adjustment:", min_value=0.0)

    if st.button("Calculate Wind Speed"):
        try:
            ...
            if data:
                ...
                if average_wind_direction is not None:
                    ...
                    if distance_meters <= max_distance:
                        ...
                        st.write(f"Distance to the found height data: {actual_distance:.2f} meters")
                    else:
                        st.write("No building height data found within the maximum search distance.")

                    st.write(f"Endpoint Coordinates: Latitude={lat2:.2f}, Longitude={lon2:.2f} ")
                    st.write(

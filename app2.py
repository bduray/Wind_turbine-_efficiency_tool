...
def calculate_wind_power(A, v):
    ...
    efficiency = 0.425  # Assumed efficiency of the turbine (42.5%)
...
# Interactive Map
st.write("### Interaktive Karte für die Platzierung der Windturbine")
st.write("Klicken Sie auf die Karte, um die Koordinaten und die Adresse der Position zu erhalten.")
...
if map_data['last_clicked'] is not None:
    ...
    # Automatically fill text input if user clicks on map
    last_selected_location = (lat, lon)
    location_text_input = reverse_geocode(lat, lon)



# Input Data
st.write("### Eingabedaten")
A = st.number_input("Windturbine Fläche (m²)", min_value=0.0, value=20.0)
h2 = st.number_input("Höhe (in Metern) für die Anpassung", min_value=0.0, value=6.0)
years = st.number_input("Anzahl der Jahre für die Nutzung der Turbine", min_value=1, value=30)

# Calculate values based on inputs
if st.button("Berechnen"):
    try:
        if last_selected_location:
            lat, lon = last_selected_location
        else:
